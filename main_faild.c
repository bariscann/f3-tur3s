...
void modMedianBul (double *tmp,double *median, double *mod)
{
...
void normalize(double *degerler, double min, double ranj)
{
	int i;

	for(i = 0; i < kontrolAdet; i++)
	{
		degerler[i] = (degerler[i]-min)/ranj;
	}
}
...
void islem(double **degerler,int baslangic, int argSayisi, char ayrac, FILE **fWrite)
{
    ...
    for(x=0; x<argSayisi; x++)
    {
        ...
        normalize(tmp,min,ranj);
        minInc = minIncBul(tmp,baslangic);
        maxInc = maxIncBul(tmp,baslangic);
        minRed = minRedBul(tmp,baslangic);
        maxRed = maxRedBul(tmp,baslangic);

        aritmetikOrtalama = ortalamaAritmetikBul(tmp);
        harmonikOrtalama = ortalamaHarmonikBul(tmp);
        kuadratikOrtalama = ortalamaKuadratikBul(tmp);
        varyans = varyansBul(tmp,aritmetikOrtalama);
        stdSapma = standartSapma(varyans);
        anlikOrtalamalar(tmp,baslangic,&anlikAritmetikOrt,&anlikHarmonikOrt,&anlikKuadratikOrt);
        varyasyonKatSayi = varyasyonKatSayisi(stdSapma,aritmetikOrtalama);
        modMedianBul(tmp,&median,&mod);
        //printf("\nMin: %lf, Max: %lf,",geometrikOrtalama,varyans);
        fprintf(*fWrite,"%lf%c%lf%c%lf%c%lf%c%lf%c%lf%c%lf%c%lf%c%lf%c%lf%c%lf%c%lf%c%lf%c%lf%c%lf%c%lf%c",minRed,ayrac,maxRed,ayrac,minInc,ayrac,maxInc,ayrac,ranj,ayrac,aritmetikOrtalama,ayrac,harmonikOrtalama,ayrac,kuadratikOrtalama,ayrac,mod,ayrac,median,ayrac,stdSapma,ayrac,varyans,ayrac,anlikAritmetikOrt,ayrac,anlikHarmonikOrt,ayrac,anlikKuadratikOrt,ayrac,varyasyonKatSayi,ayrac);
    }
    free(tmp);
...
int main()
{
    ...
    char *ozellikler[] = {"MINRED","MAXRED","MININC","MAXINC", "RANGE",
